...
final class AuthorizationClientTest extends TestCase
{
     ...
     */
    #[Test]
    public function it_retries_until_the_document_is_authorized(): void
    {
        $fakeSleeper = new FakeSleeper();

        $fakeSoapClient = new FakeSoapClient(
            authorizationResponses: [
                self::authorizationResponse(status: 'NO AUTORIZADO'),
                self::authorizationResponse(
                    status: 'AUTORIZADO',
                    accessKey: self::ACCESS_KEY,
                    xml: '<factura>Autorizada</factura>',
     ...
     */
    #[Test]
    public function it_returns_failure_after_max_attempts(): void
    {
        $fakeSleeper = new FakeSleeper();
